    ...
    <>
      <header className="site-header shell">
        ...
        <nav className="site-nav" aria-label="主导航">
          ...
          <a href={sitePath('/types/')}>人格图鉴</a>
          <a href={sitePath('/compare/')}>类型对照</a>
          <a href={sitePath('/guides/')}>方法指南</a>
          <a href={sitePath('/about/')}>关于 TT16</a>
      ...
      <main>
        <section className="hero shell">
          <div className="hero-copy">
            ...
            <div className="free-note">
              <strong>所有内容永久免费</strong>
              <span>完成后直接查看完整报告</span>
              <span>本地评分 · 不上传逐题答案</span>
            </div>
            ...
            <div className="hero-actions">
              <button className="button button--primary button--large" onClick={hasProgress ? onResume : onStart} disabled={busy}>
                {busy ? '正在读取本机进度…' : hasProgress ? `继续上次测试 · ${answeredCount}/20` : '开始认识自己'}
                <ArrowRight size={19} />
              </button>
              <button className="button button--ghost button--large hero-guess-button" onClick={onGuess}>
                让 TT16 猜猜你
                <Sparkles size={18} />
              </button>
        ...
        <section className="section shell cta-panel">
          <div className="cta-panel__copy">
            ...
            <p>20 题与完整报告全部免费；不上传答案，不连接券商账户。</p>
          </div>
          <div className="cta-panel__actions">
            <button className="button button--light button--large" onClick={onStart}>开始 20 道情境题<ArrowRight size={19} /></button>
            <button className="button button--outline-light button--large" onClick={onGuess}>先玩快速猜型<Sparkles size={18} /></button>
          </div>
      ...
      <footer className="site-footer">
        <div className="shell site-footer__inner">
          ...
          <div className="footer-links">
            <a href={sitePath('/types/')}>人格图鉴</a>
            <a href={sitePath('/methodology/')}>模型方法</a>
            <a href={sitePath('/privacy/')}>隐私说明</a>
            <a href={sitePath('/about/')}>关于 TT16</a>
            <a href={SOURCE_REPOSITORY_URL} target="_blank" rel="noreferrer">AGPL-3.0 源码</a>
            <span>TT16 Free Growth v2</span>
          </div>
        </div>
      </footer>
    </>
