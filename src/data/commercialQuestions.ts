import type { AnswerValue, DimensionKey, DimensionLetter, QuestionDirection } from './questions'

export const COMMERCIAL_ASSESSMENT_VERSION = 'tt16-commercial-1.1.0'
export const COMMERCIAL_QUESTIONNAIRE_VERSION = 'tt16-q20-1.0.0'
export const COMMERCIAL_SCORING_VERSION = 'tt16-score20-1.0.0'
export const COMMERCIAL_CONTENT_VERSION = 'tt16-content-1.0.0'

export type CommercialQuestionKind = 'dimension' | 'pressure' | 'consistency'
export type PressureKey = 'missed_rally' | 'drawdown'

interface CommercialBaseQuestion {
  id: string
  kind: CommercialQuestionKind
  prompt: string
  leftText: string
  rightText: string
  tag: string
...
export interface CommercialDimensionQuestion extends CommercialBaseQuestion {
  kind: 'dimension'
...
export interface CommercialPressureQuestion extends CommercialBaseQuestion {
  kind: 'pressure'
  pressureKey: PressureKey
}

export interface CommercialConsistencyQuestion extends CommercialBaseQuestion {
  kind: 'consistency'
...
export type CommercialQuestion =
  | CommercialDimensionQuestion
  | CommercialPressureQuestion
  | CommercialConsistencyQuestion

export const COMMERCIAL_ANSWER_SCALE: readonly { value: AnswerValue; label: string }[] = [
  { value: -2, label: '更接近 A' },
  { value: -1, label: '略接近 A' },
  { value: 0, label: '视情况' },
  { value: 1, label: '略接近 B' },
...
/**
 * Commercial 20-item assessment.
 *
 * The items are selected from the research-oriented 32-item bank, but receive
 * new IDs so commercial results can never be confused with the demo model.
 * Core dimension and consistency items are required. Pressure items allow an
 * explicit “未经历” answer and only personalize scenario modules.
 */
export const COMMERCIAL_QUESTIONS = [
  {
    id: 'C01',
    kind: 'dimension',
...
] as const satisfies readonly CommercialQuestion[]

export type CommercialQuestionId = (typeof COMMERCIAL_QUESTIONS)[number]['id']

export const COMMERCIAL_QUESTION_BY_ID = Object.freeze(
  Object.fromEntries(COMMERCIAL_QUESTIONS.map((question) => [question.id, question])),
) as Readonly<Record<CommercialQuestionId, (typeof COMMERCIAL_QUESTIONS)[number]>>

export const COMMERCIAL_DIMENSION_LETTERS: Readonly<
